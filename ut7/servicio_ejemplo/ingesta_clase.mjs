...
const df_numero = df.map(fila =>{
    if(fila.hasOwnProperty("granos_de_polen_x_metro_cubico")){
        const conversion = parseFloat(fila.granos_de_polen_x_metro_cubico)
        console.log(`${conversion}: ${typeof conversion}`)
        fila.granos_de_polen_x_metro_cubico = isNaN(conversion) ? 0 : conversion
    }
    return fila
}
)
